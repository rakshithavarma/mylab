...
$firstname = $lastname = $emailid = $section = $username = $password = $confirm_password = "";
$firstname_err = $lastname_err = $emailid_err = $section_err = $username_err = $password_err = $confirm_password_err = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty(trim($_POST["section"]))) {
        $section_err = "Please enter a last name.";
    } else {
        $section = trim($_POST["section"]);
    }
    if (empty(trim($_POST["emailid"]))) {
        $username_err = "Please enter a email id.";
    } elseif (!preg_match('/^(\d\d[0-9a-z]{8})@gvpce\.ac\.in$/ix', trim($_POST["emailid"]))) {
    ...
    if (empty($firstname_err) && empty($lastname_err) && empty($emailid_err) && empty($username_err) && empty($password_err) && empty($confirm_password_err)) {
        ...
        $sql = "INSERT INTO students (firstname, lastname, section, emailid, username, password) VALUES (?, ? , ? , ?, ?, ?)";

        if ($stmt = mysqli_prepare($link, $sql)) {
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssss", $param_firstname, $param_lastname, $param_sec, $param_emailid, $param_username, $param_password);
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            ...
            <div class="form-group">
                ...
                <select name="section" required>
                    ...
                    ?>
                        <option value="<?php echo $row['class_title']; ?>">
